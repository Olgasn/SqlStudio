...
|---|---|---|
| `MySQL` | Строка подключения к MySQL-базе приложения (курсы, лабораторные работы, задания). Используется Entity Framework Core при запуске и при выполнении команд `dotnet ef`. | Администратор перед первым запуском. |
| `LabsConnection` | Строка подключения к SQL Server — изолированной базе данных, в которой фактически выполняются студенческие и эталонные запросы. Все запросы выполняются в транзакциях, которые всегда откатываются, поэтому данные в этой базе не изменяются. | Администратор перед первым запуском. |
...
### Производственная среда — переменные окружения
